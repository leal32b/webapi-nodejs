...
import { type SignUpUserData, SignUpUserUseCase } from '@/identity/1.application/use-cases/sign-up-user-use-case'
...
const makeSignUpUserDataFake = (): SignUpUserData => ({
  email: 'any@mail.com',
  locale: 'en',
  name: 'any_name',
...
type SutTypes = {
  sut: SignUpUserUseCase
  userRepository: UserRepository
  hasher: Hasher
  encrypter: Encrypter
  errorFake: DomainError
  signUpUserDataFake: SignUpUserData
}

const makeSut = (): SutTypes => {
  const doubles = {
    errorFake: makeErrorFake(),
    signUpUserDataFake: makeSignUpUserDataFake()
  }
  const props = {
  ...
  const sut = SignUpUserUseCase.create(props)
...
describe('SignUpUserUseCase', () => {
  describe('success', () => {
    it('calls UserRepository.readByEmail with correct params', async () => {
      const { sut, userRepository, signUpUserDataFake } = makeSut()

      await sut.execute(signUpUserDataFake)

      expect(userRepository.readByEmail).toHaveBeenCalledWith('any@mail.com')
    })

    it('calls Hasher.hash with correct params', async () => {
      const { sut, hasher, signUpUserDataFake } = makeSut()
    ...
    it('calls Encrypter.encrypt with correct params', async () => {
      const { sut, encrypter, signUpUserDataFake } = makeSut()
    ...
    it('returns Right with message and email when execute succeeds', async () => {
      const { sut, signUpUserDataFake } = makeSut()

      const result = await sut.execute(signUpUserDataFake)
  ...
  describe('failure', () => {
    it('returns Left with Error when UserRepository fails', async () => {
      const { sut, userRepository, errorFake, signUpUserDataFake } = makeSut()
    ...
    it('returns Left with PasswordMismatchError when passwords do not match', async () => {
      ...
      const result = await sut.execute({ ...signUpUserDataFake, passwordRetype: 'anything' })
    ...
    it('returns Left with Error when Hasher.hash fails', async () => {
      const { sut, hasher, errorFake, signUpUserDataFake } = makeSut()
    ...
    it('returns Left with Error when Encrypter.encrypt fails', async () => {
      const { sut, encrypter, errorFake, signUpUserDataFake } = makeSut()
    ...
    it('returns Left with Error when UserAggregate.create fails', async () => {
      ...
      const result = await sut.execute({ ...signUpUserDataFake, email: 'invalid_email' })
